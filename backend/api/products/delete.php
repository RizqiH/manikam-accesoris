...
if(!empty($data->id)) {
    $product->id = $data->id;
    error_log("[DELETE] Product ID to delete: " . $product->id);
    
    if($product->delete()) {
        error_log("[DELETE] Product deleted successfully: ID " . $product->id);
        http_response_code(200);
        echo json_encode(array("message" => "Product was deleted successfully."));
    } else {
        error_log("[DELETE] Failed to delete product: ID " . $product->id);
